...
- **Søgninger/Findings**:
  ...
  - 2025-11-27 – **🔥 CRITICAL: DynamicShader parameter access** – DynamicShader does NOT expose @param fields as direct properties. Cannot use `shader.cameraPos = value`, `untyped shader.cameraPos = value`, or `Reflect.setField()`. Solution: Cast to `hxsl.DynamicShader` and use `setVariable(name: String, value: Dynamic)` method. Example: `cast(shader, hxsl.DynamicShader).setVariable("cameraPos", s3d.camera.pos)`. Parameters stored internally in `values` (vectors/objects) and `floats` (scalars) arrays with `varNames` map for lookup.
  - 2025-11-27 – **Heaps Screenshot Implementation** – No direct backbuffer capture API in Heaps. Solution: Render-to-texture approach: Create render target with `new h3d.mat.Texture(width, height, [Target])`, push render target via `engine.pushTarget()`, render scene with `s2d.render(engine)`, pop with `engine.popTarget()`, capture pixels with `texture.capturePixels()`, save with `pixels.toPNG()`. Attempting to capture bitmap tile texture only captures 1×1 pixels (solid color texture, not rendered output).
