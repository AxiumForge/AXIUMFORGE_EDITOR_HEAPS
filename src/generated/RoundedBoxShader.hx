...
class RoundedBoxShader extends hxsl.Shader {
    static var SRC = {
        ...
        function sdf(p: Vec3): Float {
            ...
            var rep_p0 = p;
            rep_p0.x = mod(rep_p0.x + 2, 4) - 2;
            rep_p0.z = mod(rep_p0.z + 2, 4) - 2;
            var cyl_d1 = abs(vec2(length(rep_p0.xz), rep_p0.y)) - vec2(0.35, 1.25);
            var box_q0 = abs(p) - vec3(0.6, 0.6, 0.6);
            var smu_a1 = length(max(box_q0, vec3(0.0))) + min(max(box_q0.x, max(box_q0.y, box_q0.z)), 0.0);
            var smu_b1 = length(p) - 1.0;
            var smu_h1 = clamp(0.5 + 0.5 * (smu_b1 - smu_a1) / 0.18, 0.0, 1.0);
            return mix(smu_b1, smu_a1, smu_h1) - 0.18 * smu_h1 * (1.0 - smu_h1);
        }
