...
class PillarRepeatShader extends hxsl.Shader {
    static var SRC = {
        ...
        function sdf(p: Vec3): Float {
            // Domain repetition
            var modP = p;
            modP.x = mod(modP.x + 2.0, 4.0) - 2.0;
            modP.z = mod(modP.z + 2.0, 4.0) - 2.0;

            // Cylinder SDF
            var d = abs(vec2(length(modP.xz), modP.y)) - vec2(0.35, 1.25);
            var cylinder = min(max(d.x, d.y), 0.0) + length(max(d, vec2(0.0)));

            // Sphere SDF
            var sphere = length(modP) - 0.35;
    ...
    public function new() {
        super();
        cameraUp = new h3d.Vector(0, -1, 0);  // Inverted Y to fix upside-down camera
        fov = 1.0;
    }
}
